...
public class Warp extends Command {
	...
	public void execute(final Game game, final HashMap<String, String> strings,
		...
		try {
			game.getMapHandler().loadMap(parseString(game, args[0], strings));
			game.getPlayer().setCoord(
					new Coord(Integer.parseInt(parseString(game, args[1], strings)),
							Integer.parseInt(parseString(game, args[2], strings))),
					Integer.parseInt(parseString(game, args[3], strings)));
