...
public class TriggerList {
	...
	// TODO: stop making copies of the scripts
	public Script[][][] load(final AssetManager assets, final MapData map, final String mapName) {
		...
		for (final Trigger trigger : triggers) {
			if (trigger.script.startsWith("$")) {
				scriptMap[trigger.layer][trigger.x][trigger.y] = new Script(assets.get("assets/scripts/" + trigger.script.substring(1) + ".ps", Script.class));
			} else {
