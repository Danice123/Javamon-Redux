...
public class Gen1Battle extends BattleMenu {
	...
	protected void init(final AssetManager assets) {
		...
		player.getPokemonTextures().forEach(number -> {
			playerPokemonTex.put(Integer.parseInt(number),
					new Texture("assets/pokemon/back/" + number + ".png"));
		});

		enemyPokemonTex = Maps.newHashMap();
		enemy.getPokemonTextures().forEach(number -> {
			enemyPokemonTex.put(Integer.parseInt(number),
					new Texture("assets/pokemon/" + number + ".png"));
		});
	}
	...
	protected void renderScreen(final RenderInfo ri, final float delta) {
		...
		final Texture playerTex = playerPokemonTex
				.get(system.getPlayerPokemon().getPokemon().number);
		batch.draw(playerTex, 20 * ri.getScale(), 35 * ri.getScale(),
				playerTex.getWidth() * 3 * ri.getScale(),
				playerTex.getHeight() * 3 * ri.getScale());

		final Texture enemyTex = enemyPokemonTex.get(system.getEnemyPokemon().getPokemon().number);
	...
	private void renderPlayerHealthBar(final RenderInfo ri, final PokeInstance poke) {
	...
	private void renderEnemyHealthBar(final RenderInfo ri, final PokeInstance poke) {
		shape.setColor(0f, 0f, 0f, 0f);
